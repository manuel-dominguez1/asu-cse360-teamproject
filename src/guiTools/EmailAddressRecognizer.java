	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// The current character must be checked against 62 options. If any are matched
				...
				if ((currentChar >= 'A' && currentChar <= 'Z') || 		// Upper case
						...
						(currentChar >= '0' && currentChar <= '9')) {	// Digit							// Hyphen
					nextState = 1;
				}
			...
			case 1: 
				...
				// The current character must be checked against 62 options. If any are matched
				// the FSM must remain in state 1
				// The first and the second check for an alphabet character the third a numeric
				// The current character must be checked against the commercial at symbol. If matched
				// the FSM must go to state 2
				// The current character must be checked against a period. If matched
				// the FSM must go to state 0
				...
				else if (currentChar == '@') {							// Commercial at
					nextState = 2;
				}
			...
			case 2: 
				...
				// The current character must be checked against 62 options. If any are matched
				// the FSM must go to state 3
				// The first and the second check for an alphabetic character the third a numeric
			...
			case 3:
				...
				// The current character must be checked against 62 options. If any are matched
				// the FSM must remain in state 3
				// The first and the second check for an alphabetic character the third a numeric
				// The current character must be checked against a hyphen. If matched
